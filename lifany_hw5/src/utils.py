...
#
# Updated: Oct 24, 2023 by Vineet Tambe
# ##################################################################### #
...
def integrateFrankot(zx, zy, pad=512):
    ...
    # Frequency grid
    [wx, wy] = np.meshgrid(np.linspace(-np.pi, np.pi, w), np.linspace(-np.pi, np.pi, h))
    absFreq = wx**2 + wy**2
...
def enforceIntegrability(N, s, sig=3):
    ...
    delta = np.asarray([[-h[2], h[5], 1], [h[1], -h[4], 0], [-h[0], h[3], 0]])
